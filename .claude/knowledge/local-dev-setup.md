...
- The cached workspace does not provide `west` or all host build dependencies. A temporary Python 3.9 virtualenv works when it includes Zephyr's `requirements-base.txt`, ZMK's `app/scripts/requirements.txt`, CMake 3.27.9 (CMake 4.x breaks this Zephyr 3.5 checkout), `protoc-wheel-0`, `protobuf`, and `grpcio-tools`; prepend that environment's `bin` directory for builds.
